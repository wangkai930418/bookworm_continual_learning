...
def train_syn_val(config,im_net,loss_net,val_loader,syn_dataloader,optimizer,n_class):
    ...
    for epoch in range(20):
        ...
        val_acc, val_top1, pred_prob,_,_ = test(config, im_net, loss_net, val_loader, True, 0)
        if val_acc>best_acc:
            best_acc=val_acc
            best_epoch=epoch
...
def main():
    ...
    with open('pkl/task_0_val.pkl', 'rb') as g:
        task_0_val = pkl.load(g)
    ...
    # task_0_train_set=grab_data(best_cfg, random.sample(task_0_train,1000), datasets[0][2], True)
    task_0_val_set = grab_data(best_cfg, task_0_val, datasets[0][2], False)

    # task_0_trainval_set = grab_data(best_cfg, datasets[0][0], datasets[0][2], True)
    ##### final testing
    task_1_test_set = grab_data(best_cfg, datasets[0][1], datasets[0][2], False)
    ...
    #seen
    #####################################################################
    task_0_real_train=get_prev_feat(FE_model, task_0_train_set, CUDA)
    ...
    task_1_fake = generate_syn_feature(50, vae, test_attr, config['syn_num'], config)

    # train_X = torch.cat((task_0_fake[0].cuda(), task_1_fake[0].cuda()))
    # train_Y = torch.cat((task_0_fake[1].cuda(), task_1_fake[1].cuda() + 150))
    train_X = torch.cat((task_0_real_train[0].cuda(), task_1_fake[0].cuda()))
    train_Y = torch.cat((task_0_real_train[1].cuda(), task_1_fake[1].cuda() + 150))
    ...
    best_loss_net=train_syn_val(config, FE_model, test_loss_net, task_0_val_set, test_dataloader,
                                test_loss_net_optimizer, 200)
    test_loss_net=copy.deepcopy(best_loss_net)
    print('\n...TESTING... GZSL: 200 labels')
    #####################################################################
    # # torch.save(test_loss_net, 'vae_time_1_loss_net.pth')
    # test_loss_net = torch.load('hist_files/vae_time_1_loss_net.pth')
    ...
    # torch.cat((true_labels_0,true_labels_1+150))
    # np.save('confusion_matrix/vae_time_1_true_labels.npy',
    #         torch.cat((true_labels_0, true_labels_1 + 150)).detach().cpu().numpy())
    #
    # np.save('confusion_matrix/vae_time_1_pred_labels.npy',
...
if __name__ == '__main__':
    ...
    # test_loss_net=torch.load('vae_time_1_loss_net.pth')
    # num_of_1st_task=150
    # cls_1_weight=torch.norm(test_loss_net.weight[:num_of_1st_task,:],dim=1).cpu().detach().numpy()
    # cls_2_weight = torch.norm(test_loss_net.weight[num_of_1st_task:,:], dim=1).cpu().detach().numpy()
    #
    # cls_1_bias=test_loss_net.bias[:num_of_1st_task].cpu().detach().numpy()
    # cls_2_bias=test_loss_net.bias[num_of_1st_task:].cpu().detach().numpy()
    #

...
def test(config, im_net, loss_net, val_loader,CUDA):
    ...
    with torch.no_grad():
        for valid_batch_num, valid_batch_data in enumerate(val_loader):
            ...
            prec1, class_acc, class_cnt, prec_prob = accuracy( output, label, config['n_train_lbl'])
...
def train(config, im_net, im_net_prev, loss_net, train_loader, optimizer, criterion, epoch, CUDA):
    ...
    for batch_num, batch_data in enumerate(train_loader):
        ...
        prec1, class_acc, class_cnt, pred_prob = accuracy(output, label, config['n_classes'])
...
def main():
    ...
    # trainval_data = datasets[0][0]

    # task_0_train=random.sample(trainval_data,len(trainval_data)-1000)
    # task_0_test=list(set(trainval_data)-set(task_0_train))
    # if not os.path.exists('pkl'):
    #     os.makedirs('pkl')
    # with open('pkl/task_0_train.pkl','wb') as f:
    #     pkl.dump(task_0_train,f)
    # with open('pkl/task_0_val.pkl', 'wb') as g:
    #     pkl.dump(task_0_val, g)
    ...
    classifier = nn.Linear(in_features=2048, out_features=150)
    ...
    FE_prev=copy.deepcopy(FE_model)
    FE_prev.eval()
    for name, para in FE_prev.named_parameters():
        para.requires_grad = False

    if CUDA:
        FE_model=FE_model.cuda()
        classifier=classifier.cuda()
        FE_prev=FE_prev.cuda()
    ...
    for epoch in range(config['epoch']):
        ...
        train(config, FE_model, FE_prev, classifier, task_0_train_set, optimizer, criterion, epoch, CUDA)
        print('\nVAL ... ')
        val_acc, val_top1, pred_prob=test(config, FE_model, classifier, task_0_val_set,CUDA)
        ...
        if is_best:
            ...
            best_top1=val_top1
            print('\n TEST...')
            test_acc, test_top1, pred_prob = test(config, FE_model, classifier, task_0_seen_test_set, CUDA)
...
if __name__ == '__main__':
    ...
    ######################  FE model parameters ##################################3
    parser.add_argument('--finetune',action='store_true',
                        help='finetune awareness')
    parser.add_argument('--distill', action='store_true',
                        help='distill awareness')
    ################################################################33
